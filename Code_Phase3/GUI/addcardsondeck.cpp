    ...
    wDeck(deck)
{
    ...
    ui->nbrOfCardsLabel->setText(labelTemplate.arg(QString::number(myClient->getBufferDeck(wDeck).getMyDeck().size())));
    ...
    if (myClient->find(stoi(txtString),myClient->getCollection().getAllCards()) && deckToModify.getSize()<20 &&  myClient->count(stoi(txtString),deckToModify.getMyDeck()) < 2 ){
        if (myClient->find(stoi(txtString),deckToModify.getMyDeck())){
            if (myClient->count(stoi(txtString),myClient->getCollection().getAllCards())== 2){
                ...
                QString labelTemplate = tr("<font style=font-size:14pt; color=#ffffff><b>%1</b></font>");
                ui->nbrOfCardsLabel->setText(labelTemplate.arg(QString::number(deckToModify.getMyDeck().size())));
                modified = true;
...
void AddCardsOnDeck::closeEvent(QCloseEvent *bar){
    ...
    if (modified && myClient->getBufferDeck(wDeck).getMyDeck().size() == 20){

        Deck toset = myClient->getBufferDeck(wDeck);
        myClient->setDeck(toset,wDeck);

        myClient->sendStringToServer("update");
        myClient->receiveFromServer();
        myClient->sendStringToServer(myClient->getUsername());
        myClient->receiveFromServer();
        myClient->sendStringToServer(to_string(wDeck));
        myClient->receiveFromServer();

        for (int i = 0;i<20;i++){
            myClient->sendStringToServer(to_string(myClient->getBufferDeck(wDeck).getMyDeck()[i]));
        }
