...
userChatManagers(vector<ChatManager*>()),gameInProgress(),numberGames(0),myServer(nullptr)
{}

Matchmaking::Matchmaking(Server* server): userInMatchmaking(vector<User*>()),userGameSockets(vector<int>()),userDecks(vector<GameDeck>()),
userChatManagers(vector<ChatManager*>()),gameInProgress(),numberGames(0), myServer(server){}

Matchmaking::~Matchmaking(){
	for (size_t i; i < gameInProgress.size(); ++i){
...
void Matchmaking::addUserInMatchmaking(User* user, int game_fd, GameDeck deck, ChatManager* cm){
	...
	if (userInMatchmaking.size() == 2){
		...
		userChatManagers.erase(userChatManagers.begin());
		++numberGames;
		createGame(user1, sockfd1, deck1, cm1, user2, sockfd2, deck2, cm2);
		// End critical
	}
}  
...
void Matchmaking::createGame(User* user1, int sockfd1, GameDeck deck1, ChatManager* cm1, User* user2, int sockfd2, GameDeck deck2, ChatManager* cm2){
	...
	if (send(sockfd2, const_cast<char*>(user2->getProfil().c_str()), MAXDATASIZE-1, 0) == -1) {
			perror("send");
			exit(1);		
	}

	Player* player1 = new Player(user1,deck1,sockfd1,cm1);
	Player* player2 = new Player(user2,deck2,sockfd2,cm2);
