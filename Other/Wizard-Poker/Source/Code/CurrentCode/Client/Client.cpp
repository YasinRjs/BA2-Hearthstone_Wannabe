...
void Client::signUp(){
	...
	while (!(strcmp(inputPlayer,""))){
		...
		if (!strcmp(msgServer,"OK")){
			...
			initialiseAllDecks(usernameToInitialise);
...
void Client::loadCollectionAndDecks(){
	receiveFromServer();
	int ID = atoi(msgServer);
	string collection;
	//Critical
	fstream fichier;
...
void Client::loadDecks(string userToLoad){
	fstream fichier;
	string username;
	string deckStr;
	...
	while(!fichier.eof() && !found){
		if (!strcmp(username.c_str(),userToLoad.c_str())){
			...
			for (int i = 0;i<5;i++){
				...
				string token;
				char delimiter = ',';
				vector<int> mydeck;//TODO DECK myDeck 
				if(deckStr.size() > 0){
...
vector<int> Client::createRandomDeck(){ //prendre l'indice de début genre il peut choisir de crérer un deck puis il le compléte aléatoirement
		...
		while (randomDeck.size()<20){
			...
			if (c == 0){
				randomDeck.push_back(randomCard);	
	        		...
	        		if (count(randomCard,myCollection.getAllCards())){
...
void Client::initialiseAllDecks(string userToInitialise){
	fstream fichier;
...
void Client::updateDecksFile(string usrname){
	rename("txt/comptesDecks.txt","txt/old.txt");	
	ifstream in("txt/old.txt");
   	ofstream out("txt/comptesDecks.txt");
	string tmp;
	while(!in.eof()){
		getline(in,tmp);
...
/*void Client::newUserDecks(){
	// --------------- NEW -----------------------//
	vector<int> basicDeck;
	for(int i=0;i<20;++i){
		basicDeck.push_back(i);
	}
	for(int i = 0; i < NUMBEROFDECK; ++i)
	{
		myDecks[i] = Deck(basicDeck);
	}
...
void Client::collectionMenu(){
	...
	cout << "- 3 : Créer un deck" << endl;
	cout << "- 4 : Supprimer un deck" << endl;
	cout << "- 5 : Modifier un deck" << endl;
	cout << "- 6 : Menu principal" << endl;
	int flag = 1;
	while (flag){
		cout << "Votre Réponse ( 1,2,3,4 ou 5 ou 6) : ";
		cin >> inputPlayer;
	...
	else if (!strcmp(inputPlayer,"3")){//TODO
		//int chosenDeck = choseDeck();
		//createNewDeckNumber(chosenDeck);
