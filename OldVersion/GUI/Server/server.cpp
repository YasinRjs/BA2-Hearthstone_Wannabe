...
ClientSocket* Server::acceptConnection(){
    int new_fd, rcvMsgChat_fd, sendMsgChat_fd, game_fd;
    ...
    if ((game_fd = accept(serverfd,reinterpret_cast<struct sockaddr *>(&addr),&sin_size)) == -1){
        perror("accept");
        cout<<"game_fd"<<endl;
        exit(1);
    }
    return new ClientSocket(addr,new_fd, rcvMsgChat_fd, sendMsgChat_fd, game_fd);
}
