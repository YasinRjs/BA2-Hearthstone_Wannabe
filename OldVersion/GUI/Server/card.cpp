...
string Card::parseCard(){
    string card;
    card += to_string(type.id);
    card += ";" + type.name;
    card += ";" + to_string(type.energyCost);
    card += ";" + to_string(type.attack) + ";" + to_string(type.hp);
    return card;
}
...
string Card::guiParseCard(){
    string parsedCard;
    parsedCard += type.name + ";";
    parsedCard += to_string(type.energyCost) + ";";
    parsedCard += to_string(type.attack) + ";";
    parsedCard += to_string(type.hp);
    return parsedCard;
}

string Card::guiParserCard(){
    string parsedCard;
    parsedCard += type.name +" "+to_string(type.id)+ ";";
    parsedCard += to_string(type.energyCost) + ";";
    parsedCard += to_string(type.attack) + ";";
    parsedCard += to_string(type.hp);
    return parsedCard;
